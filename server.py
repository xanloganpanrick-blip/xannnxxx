...
def normalize_fio_local(raw):
    """РџСЂРёРІРѕРґРёС‚ Р¤РРћ Рє С„РѕСЂРјР°С‚Сѓ 'РљР°Р¶РґРѕРµ РЎР»РѕРІРѕ РЎ РџСЂРѕРїРёСЃРЅРѕР№'"""
    if not raw:
        return ""
    cleaned = re.sub(r'[^A-Za-zРђ-РЇРЃР°-СЏС‘\-]', ' ', str(raw))
    words = [w.strip() for w in cleaned.split() if w.strip()]
    if not words:
        return ""
    # РљР°Р¶РґРѕРµ СЃР»РѕРІРѕ СЃ РїСЂРѕРїРёСЃРЅРѕР№, РѕСЃС‚Р°Р»СЊРЅС‹Рµ СЃС‚СЂРѕС‡РЅС‹Рµ
    return ' '.join([w[0].upper() + w[1:].lower() if w else '' for w in words])

def normalize_address_local(raw):
    """РџСЂРёРІРѕРґРёС‚ Р°РґСЂРµСЃ Рє С„РѕСЂРјР°С‚Сѓ 'Р“РѕСЂРѕРґ, РЈР»РёС†Р°, Р”РѕРј, РљРІР°СЂС‚РёСЂР°' СЃ РїСЂРѕРїРёСЃРЅРѕР№ Р±СѓРєРІС‹"""
    if not raw:
        return ""
    s = str(raw).strip().lower()
    # РЈРґР°Р»СЏРµРј Р»РёС€РЅРёРµ РїСЂРµС„РёРєСЃС‹: РѕР±Р»Р°СЃС‚СЊ, РіРѕСЂРѕРґ, СѓР»РёС†Р° Рё С‚.Рґ.
    s = re.sub(r'\b(РѕР±Р»Р°СЃС‚СЊ|РѕР±Р»|РєСЂР°Р№|СЂРµСЃРїСѓР±Р»РёРєР°|Р°Рѕ|СЂР°Р№РѕРЅ|СЂ-РЅ)\b\.?\s*', '', s)
    s = re.sub(r'\b(Рі|РіРѕСЂРѕРґ|РіРѕСЂ)\b\.?\s*', '', s)
    s = re.sub(r'\b(СѓР»|СѓР»РёС†Р°|РїСЂ-С‚|РїСЂРѕСЃРїРµРєС‚|РїРµСЂ|РїРµСЂРµСѓР»РѕРє|РїСЂ|РїСЂРѕРµР·Рґ|Р±-СЂ|Р±СѓР»СЊРІР°СЂ|РїР»|РїР»РѕС‰Р°РґСЊ|РЅР°Р±|РЅР°Р±РµСЂРµР¶РЅР°СЏ|С€|С€РѕСЃСЃРµ)\b\.?\s*', '', s)
    s = re.sub(r'\b(Рґ|РґРѕРј|РІР»Рґ|РІР»Р°РґРµРЅРёРµ)\b\.?\s*', '', s)
    s = re.sub(r'\b(РєРІ|РєРІР°СЂС‚РёСЂР°|РєРІ-СЂР°)\b\.?\s*', '', s)
    s = re.sub(r'\b(РєРѕСЂРї|РєРѕСЂРїСѓСЃ|СЃС‚СЂ|СЃС‚СЂРѕРµРЅРёРµ)\b\.?\s*', '', s)
    s = re.sub(r'\s+', ' ', s).strip()
    s = re.sub(r'\s*,\s*', ', ', s)
    # РљР°Р¶РґРѕРµ СЃР»РѕРІРѕ СЃ РїСЂРѕРїРёСЃРЅРѕР№ Р±СѓРєРІС‹
    parts = []
    for p in s.split(','):
        p = p.strip()
        if p:
